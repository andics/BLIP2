...
cc3m_dir = "/home/projects/bagon/dannyh/data/seedbench_filt/Constant_10d"
...
def run_inference(model, qa_anno, output_dir):
    ...
    with open(os.path.join(output_dir, "results.json"), mode="r", encoding="utf-8") as f:
        while True:
            ...
            if ans == '':
                break
            prev_answer_list.append(json.loads(ans))
            prev_quest_list.append(json.loads(ans)['question_id'])
...
#    if not os.path.exists(os.path.join(output_dir, "results.json")):
    ...
    for qa_item in tqdm(qa_anno):
        ...
        answer_record = {
            ...
            'prediction': prev_answer_list[prev_quest_list.index(qa_item['question_id'])]['prediction'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', type=str, default=cc3m_dir)
